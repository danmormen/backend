...
// Formato de fecha AAAA-MM-DD para MySQL
...
// 1. APLICAR PROTECCIÓN GENERAL (Requiere Token)
// Todas las rutas de este archivo necesitan que el usuario esté logueado
router.use(protect);

/**
 * PATCH /api/usuarios/:id/cambiar-password
 * UBICACIÓN CRÍTICA: Debe estar ANTES de adminOnly.
 * Permite a los usuarios cambiar su propia contraseña o a los administradores.
 */
router.patch('/:id/cambiar-password', async (req, res) => {
    const { id } = req.params;
    const { password } = req.body;

    // Usamos 'req.user' porque así lo definimos en el middleware corregido
    ...
    try {
        ...
        // Al cambiar la contraseña, ponemos requiere_cambio en 0 para que no se lo pida de nuevo
...
// A partir de aquí, todas las rutas requieren rol de 'admin'
...
/**
 * POST /api/usuarios - Crear nuevo usuario (Asigna requiere_cambio = 1 por defecto)
 */
router.post('/', async (req, res) => {
    const { nombre, email, password, telefono, rol = 'estilista', especialidad, direccion, fecha_nacimiento, avatar, activo = 1 } = req.body;
    ...
    try {
        ...
        const sql = `
            INSERT INTO usuarios (nombre, email, password, telefono, rol, especialidad, direccion, fecha_nacimiento, avatar, activo, requiere_cambio) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)
        `;

        const values = [nombre.trim(), email.trim(), hashedPassword, telefono || null, rol, especialidad || null, direccion || null, fechaSQL, avatar || null, activo];

        db.query(sql, values, (err, result) => {
            if (err) {
                if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'El correo ya está registrado.' });
...
router.put('/:id', (req, res) => {
    ...
    const sql = `UPDATE usuarios SET nombre = ?, email = ?, telefono = ?, rol = ?, especialidad = ?, direccion = ?, fecha_nacimiento = ?, avatar = ?, activo = ? WHERE id = ?`;
    const values = [nombre.trim(), email.trim(), telefono || null, rol, especialidad || null, direccion || null, fechaSQL, avatar || null, activo, id];

    db.query(sql, values, (err) => {
        if (err) return res.status(500).json({ error: 'No se pudo actualizar el usuario.' });
